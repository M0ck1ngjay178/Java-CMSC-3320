...
public class Average{
	...
	public static void main(String[] args) throws IOException
	{
		...
		double avggrade = 0;	//variable to store calculated average of total grades
		double numResult = 0;	//variable to store calculated average of total grades, seperately used for valid check
		...
		while(grade >= 0.0 && grade <= 100.0){
			...
			line = stdin.readLine();
		   try{
			grade = Double.parseDouble(line); //parse input for a double
		   }catch (NumberFormatException e) {  //if it doesnt return a double, keep gradecount and iteration the same for next loop
		...
		if (gradecount==0){ //check if there were any valid grades entered
			System.out.println("There were no valid grades");
		}
		//--------------------END CALCULATIONS---------------------------------------
