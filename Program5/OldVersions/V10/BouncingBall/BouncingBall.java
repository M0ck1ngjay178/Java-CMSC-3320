...
public class BouncingBall extends Frame implements WindowListener, ComponentListener, ActionListener, AdjustmentListener, Runnable, MouseListener,MouseMotionListener {
    ...
    private void MakeSheet(){
     ...
     public void adjustmentValueChanged(AdjustmentEvent e) {
        //somthing major wrong here ugh speen not increasng and size not either?!?!?!
         int i;
         boolean ok;
         Rectangle t;
         int half;
   ...
   //===============================================================================
     ...
     class Ballc extends Canvas{
         ...
         private void move() {
            //  Ball.x += dx;
            //  Ball.y += dy;
            if (Ball.y + (SBall-1)/2 >= ScreenHeight || Ball.y-(SBall-1)/2<=0 ) {
                dy = -dy;
            }
            if (Ball.x + (SBall-1)/2 >= ScreenWidth || Ball.x-(SBall-1)/2<=0) {
                dx = -dx;
            }
            Ball.x += dx;//update position of x
            Ball.y += dy;//update position of y
