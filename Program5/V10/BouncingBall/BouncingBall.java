...
public class BouncingBall extends Frame implements WindowListener, ComponentListener, ActionListener, AdjustmentListener, Runnable, MouseListener,MouseMotionListener {
    ...
    Button Run, Pause, Quit;

    private Panel sheet = new Panel();
    private Panel control = new Panel(); //add panels

    //objects
    private Ballc Obj; //object to draw
    ...
    public BouncingBall() {
        ...
        Obj = new Ballc(50, ScreenWidth, ScreenHeight); // Example constructor for Objc
        MakeSheet();
        try {
            initComponents();
        } catch (Exception e) {
    ...
    public void initComponents() throws Exception {
        ...
        Pause.setEnabled(false); // Disable Pause button initially
    //set delay
        delay=100-SPEED+1;
...
//        sheet = new Panel();					//for graphics canvas
...
//        control = new Panel();					//for control scrollbars, buttons, labels
...
/*        Obj.setBackground(Color.white);
        ...
        //Obj.setPreferredSize(new Dimension(ScreenWidth, ScreenHeight));  // Set the preferred size of the canvas
        sheet.add(Obj, sheetConstraints);  // Add Obj (the canvas) to the sheet
*/
        ...
        gbc.gridx = 0; gbc.gridy = 0; buttonPanel.add(Run, gbc);
        gbc.gridx = 1; buttonPanel.add(Pause, gbc);
        gbc.gridx = 2; buttonPanel.add(Quit, gbc);
 ...
 */
       ...
       buttonPanelConstraints.fill = GridBagConstraints.HORIZONTAL;
        ...
        control.add(Run, buttonPanelConstraints);
        //ADDS PAUSE
        buttonPanelConstraints.gridx = 2;
        buttonPanelConstraints.gridy = 0;
        buttonPanelConstraints.gridwidth = 1; // Span across columns
        buttonPanelConstraints.weightx = 1;  // Let it expand if needed
        control.add(Pause, buttonPanelConstraints);
        //ADDS QUIT
 ...
 //       add(control, BorderLayout.SOUTH);  // Control panel at the bottom
    ...
    private void SizeScreen() {
        ...
        ScreenWidth = WinWidth - I.left - I.right;
       // ScreenHeight = WinHeight - I.top - I.bottom - control.getHeight();
        ScreenHeight = WinHeight - I.top - 2 * (BUTTONH + BUTTONHS) - I.bottom;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(source==Run){
            ...
            /* 
            if(Run.getLabel()=="Pause"){ //if the button is paused
                Run.setLabel("Run");  //set the label to run              
                TimerPause = true;//set timer to true to stop thread
            }
            else{
                Run.setLabel("Pause"); //otherwise, set label back to pause
        ...
        if(source==Pause){
            ...
            /* 
            if(Pause.getLabel()=="Pause"){ //if the button is paused
                Pause.setLabel("Run");  //set the label to run              
                TimerPause = true;//set timer to true to stop thread
            }
            else{
                Pause.setLabel("Pause"); //otherwise, set label back to pause
                TimerPause = false; //set timer back to false to restat the thread
                started =true;     //reset stated to true
            }
            */
        ...
        if(source==Quit){//if source is equal to quit button
            runBall=false;//set runBall to false to stop the ball
    ...
    public void adjustmentValueChanged(AdjustmentEvent e) {
        ...
        if (sb == ObjSizeScrollBar) {
            ...
            //create rectangle one pixel larger than ball??
    ...
    public void mouseReleased(MouseEvent e){
      //  list.add("Mouse Button "+e.getButton()+"realeased");
    }
    ...
    class Ballc extends Canvas{
        ...
        public void paint(Graphics cg){
            ...
            g.fillOval((int)Obj.getX(), Obj.getY(), Obj.getWidth(), Obj.getHeight());
            //update(g);
        ...
        public void update(Graphics g){
            ...
            if(clear){ //if passed clear,
                ...
                //clear=false;//set clear back to false for next iteration //commented out to prevent flickering
            }
