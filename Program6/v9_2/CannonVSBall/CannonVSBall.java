...
public class CannonVSBall implements ActionListener, WindowListener, ItemListener, ComponentListener, AdjustmentListener, Runnable, MouseListener,MouseMotionListener {
    ...
    public void initComponents() throws Exception {
        ...
        //add menu item quit with shortcut key to menu entry SLOWNTROL
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(source== RUN){
            TimerPause = false; //set timer false to start the thread
        }
        if(source== PAUSE){
            TimerPause = true; //set timer to true to pause the thread
        }
        if(source== RESTART){
            TimerPause = true; //set timer false to start the thread
            //Ball.repaint();
            Ball.resetBall(0,0);
        }
            ...
            int cannonCenterX = Ball.getWidth() - 37;
             ...
             if (p.distance(cannonCenterX, cannonCenterY) <= cannonRadius) {
                if (!ProjectileActive) { // If no Projectile is active, launch a new one
                    ...
                    launchProjectile();// launches the Projectile from cannon
                }
            }
...
//========================CLASS CANVAS=================================
     //create a class to draw the canvas for the frame
      class Ballc extends Canvas{
        ...
        private boolean hitCannon = false;
        ...
        public void moveProjectile(double deltaTime){
            ...
            // Let the projectile fall back down from the top
